...
async fn ipc_feedback_flow() {
    ...
    let ok = resp
        ...
        .and_then(|v| v.as_bool())
        .expect("ok field");
    assert!(ok, "feedback should succeed");
    ...
    assert_eq!(stored_feedback.len(), 1, "should have 1 feedback record");
    assert_eq!(stored_feedback[0].error_hash, error_hash);
    assert_eq!(
        stored_feedback[0].verdict,
        organism_knowledge::Verdict::Accepted
    );
    assert_eq!(stored_feedback[0].note, Some("looks good".to_string()));
